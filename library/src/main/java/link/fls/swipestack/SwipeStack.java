...
public class SwipeStack extends ViewGroup {
    ...
    public SwipeStack(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mScreenHeight = DisplayUtils.getScreenHeightPixels(context);
        mScreenWidth = DisplayUtils.getScreenWidthPixels(context);
        Log.e("aaa", "mScreenHeight  " + mScreenHeight + "  mScreenWidth " + mScreenWidth);
    ...
    private void initialize() {
        ...
        mSwipeHelper = new SwipeHelper(this, mScreenHeight, mScreenWidth);
...
//                requestLayout();
    ...
    private void addNextView() {
        ...
        bottomView.setTag(R.id.position, mCurrentViewIndex);
...
//                mSwipeHelper.resetViewPosition();
            }
            ...
            if (x % mNumberOfStackedViews == 0) {
                childView.setRotation(mViewRotation);
            } else if (x % mNumberOfStackedViews == 1) {
                childView.setRotation(-mViewRotation * 4 / 5);
            } else if (x % mNumberOfStackedViews == 2) {
    ...
    private void removeTopView() {
        ...
        //0---9; 7发add数据
        if (getCurrentPosition() == mAdapter.getCount() - 3) {
            if (mListener != null) {
                mListener.onLastThreeData(getCurrentPosition());
            }
        }


        Log.e("ccc", "getChildCount  " + getChildCount() + "   mCurrentViewIndex  " + mCurrentViewIndex + "    getCurrentPosition()   " + getCurrentPosition() +
...
//        super.measure(widthMeasureSpec,heightMeasureSpec );
    ...
    public void onViewSwipedToTop() {
        if (mListener != null) {
            mListener.onViewSwipedToTop(getCurrentPosition());
        }
        removeTopView();
    }
    ...
    public void onViewSwipedToBottom() {
        if (mListener != null) {
            mListener.onViewSwipedToBottom(getCurrentPosition());
        }
        removeTopView();
    }
    ...
    public int getCurrentPosition() {
        int tag = (int) mTopView.getTag(R.id.position);
        return tag;
    }
    ...
    public void onClickListener() {
        if (mListener != null) {
            Log.e("ccc", "onClickListener    mCurrentViewIndex " + mCurrentViewIndex + "  getCurrentPosition  " + getCurrentPosition());
            mListener.onClickListener(getCurrentPosition());
        }
    }
     ...
     */
    public SwipBaseAdapter getAdapter() {
        return mAdapter;
    }
     ...
     * @see #getAdapter()
     */
    public void setAdapter(SwipBaseAdapter adapter) {
        if (mAdapter != null) {
    ...
    public void resetStack2Position() {
//        ViewGroup bottomView = (ViewGroup) getTopView();
//        int width = getWidth() - (getPaddingLeft() + getPaddingRight());
//        int height = getHeight() - (getPaddingTop() + getPaddingBottom());
//        LayoutParams params = bottomView.getLayoutParams();
//        if (params == null) {
...
//        bottomView.measure(measureSpecWidth | width, measureSpecHeight | height);
    ...
    public interface SwipeStackListener {
        ...
        /**
         * The last 3 pieces of data
         */
        void onLastThreeData(int position);
